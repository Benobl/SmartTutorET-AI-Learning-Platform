...
async function debug() {
    ...
    if (!tutor) {
        ...
        const allUsers = await User.find({ role: "tutor" }).select("email");
        console.log("Existing tutors:", allUsers.map(u => u.email));
        process.exit(1);
    }
    ...
    subjects.forEach(s => console.log(` - ${s.title} (Grade: ${s.grade}, Tutor ID Type: ${typeof s.tutor}, Tutor ID: ${s.tutor})`));
    
    const students = await User.find({ role: "student" });
    console.log("Total students in DB:", students.length);
    students.forEach(s => console.log(` - ${s.name} (${s.email}, Grade: ${s.grade}, Status: ${s.accountStatus}, Role: ${s.role})`));
