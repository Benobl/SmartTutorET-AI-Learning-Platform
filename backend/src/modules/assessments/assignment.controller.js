    ...
    static async createAssignment(req, res, next) {
        try {
            ...
            if (!subjectId || !title || !dueDate) {
                throw new ApiError(400, "subjectId, title, and dueDate are required.");
            }

            const subject = await getSubjectOrThrow(subjectId);
            if (req.user.role === "tutor" && String(subject.tutor) !== String(req.user._id)) {
            ...
            await NotificationService.notifyEnrolledStudents(
                subject,
                `New assignment posted in "${subject.title}": ${assignment.title} (Due: ${parsedDueDate.toLocaleDateString()})`,
                "new_assignment"
            );
